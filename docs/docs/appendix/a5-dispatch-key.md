...
> 本附录对应主�?Ch2/Ch10。Ch10 实现了简单的 device 路由 dispatcher（CPU/CUDA），本附录讲�?PyTorch 完整�?dispatch key 体系——它�?PyTorch 能同时支持多设备、autograd、autocast 的架构基石�?

---

## A5.1 为什么需�?dispatch key
...
| CPU 张量 | `cpu_add` |
| CUDA 张量 | `cuda_add` |
| 需要梯�?| `autograd_add`（先建图再调底层�?|
| 混合精度 | `autocast_add`（先�?fp16 再调�?|
| 量化推理 | `quantized_add`（int8 kernel�?|
| 元数据推�?| `meta_add`（只�?shape/dtype，不算数据） |

如果�?if/else 硬编码，每个算子都要�?N 个分支，组合爆炸�?*dispatch key** 把这些维度正交化，每个维度一�?key，按优先级查表�?
...
**问题**�?
- 每个算子都要�?N×M 个分支（N 设备 × M 功能�?
- 新增一个维度（如新设备 XLA）要改所有算�?
- 代码重复、易�?

**dispatch key 的解�?*：每个维度独立，查表组合�?

---

## A5.2 minitorch 的简�?dispatcher（Ch10 回顾�?
...
// a.device == CPU �?�?cpu_add
// a.device == CUDA �?�?cuda_add
```

**只有一�?key**（device），够用于教学。PyTorch �?dispatcher �?*十几�?key**�?

### A5.2.1 minitorch dispatcher 的实�?
...
## A5.3 PyTorch �?dispatch key 体系

### A5.3.1 key 的分�?

PyTorch �?dispatch key 分为几大类，�?*优先级从高到�?*排列�?

```
┌─ 功能 key（Functionality Keys）─────────────────────────�?
�? AutocastCUDA    �?混合精度自动转换                     �?
�? AutocastCPU     �?CPU 混合精度                         �?
�? PythonTLSSnapshot �?Python 线程局部状�?               �?
�? Python          �?Python fallback                      �?
�? Functionalize   �?函数化变换（vmap、反向模�?AD�?      �?
�? Autograd        �?自动微分（建�?+ 反向�?              �?
�? Negative        �?负数处理                              �?
├─ 后端 key（Backend Keys）──────────────────────────────�?
�? CPU             �?CPU 实现                             �?
...
�? CompositeImplicitAutograd �?�?autograd 公式组合       �?
...
当调�?`torch.add(a, b)` 时，dispatcher 收集输入张量的所�?key，按优先级从高到低查找：
...
**关键**：Autograd key 的优先级高于 CUDA，所�?`torch.add` 先走 autograd（建图），再�?CUDA（计算）�?

### A5.3.3 DispatchKeySet

每个张量有一�?key 集合（`DispatchKeySet`），�?bitmask�?
...
**bitmask 的优�?*：O(1) 检�?key 是否存在，O(1) 添加/删除 key�?
...
**对比 minitorch**：我们手动分�?`ops::add`（纯计算）和 `autograd::add`（建图），用户要选调哪个。PyTorch �?dispatch key 让用户只�?`torch.add`，autograd 自动包装�?

### A5.4.2 不同算子�?autograd 策略

| 策略 | 含义 | 示例 |
|------|------|------|
| **AutogradImpl** | 手动注册 backward | `add` �?`AddBackward` |
| **CompositeImplicitAutograd** | 用已有算子组�?| `softplus = log(1 + exp(x))`，自动用 `log`/`exp` �?backward |
| **CompositeExplicitAutograd** | 需手动注册但非自动 | `dropout`（有随机性，不能自动组合�?|

```python
# CompositeImplicitAutograd 示例
# softplus 只需注册前向，backward 自动�?log/exp �?backward 组合
...
Tensor autograd_add(const Tensor& a, const Tensor& b) {
    ...
    //                         �?去掉 Autograd
    // �?现在只查 CPU/CUDA key，不会再次进�?autograd_add
...
**如果不去�?*：`autograd_add` 调底�?�?底层又查�?Autograd key �?又调 `autograd_add` �?无限递归�?

---

## A5.5 Meta key：shape 推导

Meta key 的实�?*不计算数�?*，只推导 shape/dtype/strides�?
...
**用�?*�?
- `torch.empty_like(x)`：先�?Meta 推导 shape，再分配内存
- `torch.compile`：编译时�?Meta 推导所有中�?shape，不跑真数据
- 异构计算：先检�?shape 兼容性，再分派到 CPU/CUDA

### A5.5.1 Meta 的实际应�?

```python
# torch.compile �?Meta 推导 shape
@torch.compile
def fn(x):
...
# 编译�?
#   1. 创建 Meta 张量（只�?shape，无数据�?
#   2. 跑前�?�?推导所有中�?shape
#   3. �?shape 信息生成优化代码（Triton kernel�?
#   4. 真数据用生成的代码执�?
```
...
Functionalize �?PyTorch 最复杂�?dispatch key，用于：

- **vmap**：自动批量变�?
- **反向模式 AD**（functorch）：高阶自动微分
- **view 消除**：把 view 操作变成 clone + 计算，简化语�?
...
result = batched_fn(torch.randn(1000))  # 一次调用处�?1000 �?
...
Functionalize 通过**重写算子语义**实现这些变换，是 PyTorch 2.0 �?functorch 基础�?
...
# view 操作（共享存储）
x = torch.randn(4, 4)
y = x.view(16)      # y �?x 共享存储
y[0] = 999          # 修改 y 也修�?x

# Functionalize �?view 变成 clone:
# y = x.view(16) �?y = x.clone().reshape(16)
# �?y �?x 独立，修�?y 不影�?x
# �?简化了 autograd 的语义（不用处理 view 的梯度）
```

**为什么需�?view 消除**：view 操作的梯度很复杂（需要处理别名关系），Functionalize �?view 变成 clone 后，autograd 只需处理简单的不共享情况�?

---

## A5.7 Autocast key

混合精度训练时，Autocast key 自动把输入转�?fp16�?

```python
with torch.autocast(device_type="cuda"):
    # 在这个上下文里，算子自动�?AutocastCUDA key
    x = torch.randn(3, 3)  # fp32
    y = torch.matmul(x, x)  # �?Autocast �?x �?fp16，调 cuda_matmul_fp16
    # 某些算子（如 softmax）保�?fp32
...
**哪些算子�?fp16**：matmul、conv、linear（计算密集型�?
**哪些保持 fp32**：softmax、layer_norm、loss（数值敏感型�?

### A5.7.1 Autocast 的算子分�?
...
**分类原则**�?
- **计算密集�?*（matmul, conv）：fp16 加速明显，精度损失可接�?
- **数值敏感型**（softmax, layer_norm）：fp32 保持数值稳�?
- **降精度型**（sum, mean）：�?fp16 下累加可能溢出，保持 fp32
...
6. 结果沿调用栈返回:
   CUDA result �?Autograd �?grad_fn �?Autocast 返回
```
...
KernelFn DispatchTable::lookup(DispatchKeySet keys) const {
    // 按优先级遍历 key
    for (auto key : keys.ordered_keys()) {  // 从高优先级到�?
...
不是每个算子都注册了所�?key�?*fallback** �?没找到时的默认行�?�?
...
| key | fallback | 含义 |
|-----|---------|------|
| CUDA | CPU | �?CUDA 实现就跑 CPU（慢但能出结果） |
| Autograd | �?| �?autograd 实现则不建图 |
| Meta | �?| �?Meta 实现则不推导 shape |
| CompositeImplicitAutograd | 用前向组�?| 自动用已有算子的 backward |

### A5.9.1 CPU �?CUDA fallback

```cpp
// 没注�?CUDA 实现的算子，fallback �?CPU
...
y = torch.matmul(x, x)  # �?自动�?NPU key
```

**工作�?*：需要为所有算子实�?NPU 版本（调�?NPU 驱动 API）。这�?PyTorch 支持新硬件的标准方式�?

### A5.11.2 添加新功�?

以添�?"DebugTrace"（记录所有算子调用）为例�?
...
// 3. 启用
// 张量�?key_set 包含 DebugTrace �?自动记录所有调�?
```

**关键**：新功能不需要改任何已有算子，只需注册�?key 的实现。这�?dispatch key 的核心价值—�?*开放扩�?*�?

### A5.11.3 实际案例: PyTorch �?PrivateUse1

PyTorch 预留�?`PrivateUse1` / `PrivateUse2` / `PrivateUse3` 三个 key 供第三方扩展�?

```cpp
// 第三方后端（如华�?NPU）用 PrivateUse1
...
**这就�?torch_npu（华为昇腾）、torch_mlu（寒武纪）等第三方后端的实现方式**——不需要改 PyTorch 源码，只�?`PrivateUse1` key 注册�?
...
|------|-----------|---------|
| key 数量 | 1（device�?| ~20 |
| 查找 | `dict[op_name][device]` | `DispatchTable` �?key 优先�?|
| autograd | 手动�?`autograd::add` | 自动（Autograd key�?|
| autocast | 未实�?| AutocastCUDA key |
| meta | 未实�?| Meta key |
| fallback | 未实�?| 多级 fallback |
| 注册 | `REGISTER_CPU_OP` �?| `TORCH_LIBRARY_IMPL` �?|
| key 表示 | `enum Device` | `DispatchKeySet` (bitmask) |

### A5.12.1 为什�?minitorch 只做 device key

教学目标下，device key 足以讲清 dispatcher 的核心思想�?
- **op_name �?(key �?kernel)** 的查表结�?
- 新增 device 只需注册�?kernel，不改调用点
- 解�?算子�?�?实现"

其他 key（Autograd、Autocast 等）是工程扩展，原理相同——只�?key 更多、优先级更复杂�?

### A5.12.2 �?minitorch �?PyTorch 的扩展路�?

```
minitorch (Ch10):
  key = {CPU, CUDA}
  �?用户手动�?ops::add �?autograd::add
...
每一步都�?*添加�?key**，不改已有代码。这就是 dispatch key 架构的扩展性�?
...
| 注册�?| `TORCH_LIBRARY_IMPL` | `aten/src/ATen/library.h` |
...
# �?�?C++ keys: 未找�?
# �?�?Python key: 找到 my_op_python
```

**用�?*�?
- 快速原型（�?Python 实现，验证正确性，再写 C++�?
- 不需要性能的算�?
- �?Python 库交互（�?numpy�?
...
# 内部:
# 1. �?Negative key �?找到 negative_index_select
# 2. negative_index_select �?-1 转成 9
# 3. 调底�?index_select(x, 9)
```

**为什么需要单独的 key**：负索引的转换逻辑对所有算子相同，用一�?key 统一处理，避免每个算子都写负索引检查�?
...
3. 展开�?`log(1 + exp(x))`——调�?`log` �?`exp`
4. `log` �?`exp` 各自�?Autograd key，建�?
5. 最终的图是 `log` �?`exp` 的组合，backward 自动链式法则

### A5.15.2 CompositeExplicitAutograd

```cpp
// 需要手动注�?backward（不能自动组合）
...
**为什�?dropout 不能自动组合**：dropout 有随机性，`dropout(x) = x * mask / p`，mask 是随机的。backward 需�?*同一�?mask**，不能重新随机。所以必须手动注�?backward，保�?mask�?

---

## A5.16 dispatch key 的历史演�?

```
PyTorch 0.x:  �?dispatcher，每个算子手�?if/else 判断 device
PyTorch 1.0:  引入 dispatcher，最初只�?CPU/CUDA key
PyTorch 1.5:  加入 Autograd key（之�?autograd 是单独的层）
PyTorch 1.8:  加入 Meta key（用�?torch.compile 预研�?
PyTorch 1.10: 加入 Functionalize key（functorch�?
PyTorch 1.13: 加入 Autocast key（之�?autocast �?Python 层）
PyTorch 2.0:  dispatcher 成熟，支�?torch.compile
```

**演进动机**�?
- 早期：快速迭代，if/else 够用
- 中期：设�?功能增多，组合爆�?�?需�?dispatcher
- 后期：torch.compile 需要精确控制算子分�?�?dispatcher 成为核心

---

## A5.17 小结

PyTorch �?dispatch key 体系是一�?*多维分派�?*�?

1. **功能 key**（Autograd、Autocast、Functionalize）：包装/变换算子语义
2. **后端 key**（CPU、CUDA、Meta）：不同设备的实际实�?
3. **复合 key**（CompositeImplicitAutograd）：用已有算子自动组�?

调用 `torch.add(a, b)` 时，dispatcher 收集 a 的所�?key，按优先级从高到低查找。Autograd 先于 CUDA，所以自动建图；Autocast 先于 Autograd，所以自动转精度�?

minitorch 只实现了 device key（CPU/CUDA），但核心结构（op_name �?key �?kernel）与 PyTorch 一致。理解了这个结构，就理解�?PyTorch 如何用一�?API 支持多设备、autograd、量化、混合精度�?

**关键设计思想**�?
- **正交�?*：每个维度一�?key，避免组合爆�?
- **优先�?*：功�?key > 后端 key，先变换再计�?
- **开放扩�?*：新增设�?功能只需注册�?key，不改已有代�?
- **fallback**：未注册�?key 有默认行为，保证可用�?
- **bitmask**：key 集合�?bitmask，O(1) 操作
- **PrivateUse1-3**：预�?key 供第三方后端扩展（torch_npu 等）
