# A3 TorchScript �?ONNX export

> 本附录对应主�?Ch13（图与编译导论）。TorchScript �?PyTorch 的第一代图编译方案，已�?`torch.compile` 取代；ONNX 是跨框架模型交换格式，至今广泛使用�?

---

## A3.1 TorchScript：把动态模型变静�?

### A3.1.1 为什么需�?TorchScript

PyTorch 模型�?Python 代码——部署到 C++ 环境（如手机、嵌入式、服务器 C++ 后端）时，没�?Python 解释器。TorchScript �?Python 模型**编译成一个可序列化的�?*，脱�?Python 运行�?

```
Python 模型 �?TorchScript 编译 �?ScriptModule �?序列�?�?C++ 加载执行
```
...
用示例输入跑一次前向，**记录所有算子调�?*，构建图�?

- 优点：简单，不需要改代码
- 缺点�?*控制流被固化**——if/for 按示例输入的路径走，另一条路径丢�?
...
- 优点：保留完整控制流
- 缺点：需要代码符�?TorchScript 子集（类型注解、无动态属性等�?
...
| 情况 | 推荐 | 原因 |
|------|------|------|
| 无控制流（纯算子序列�?| trace | 简单，无需改代�?|
| 控制流依赖输入�?| script | trace 会丢分支 |
| 控制流依�?shape（非值） | trace | shape �?trace 时已知，不是 data-dependent |
| 复杂 Python 语法 | trace | script 可能不支�?|
...
TorchScript 编译后得�?`ScriptModule`，它是一�?*图结�?*而非 Python 代码�?
...
### A3.1.7 TorchScript 的子集限�?

TorchScript 不支持所�?Python 语法�?
...
| nn.Module 子类 | 多继承、元�?|
| 类型注解的函�?| 可变参数 *args/**kwargs（部分支持） |
| List[Tensor] 等容�?| 容器内混合类�?|
...
def my_fn(x: Tensor, n: int) -> Tensor:
    ...
    for i in range(n):       # �?n 必须�?int，不能是 Tensor
        result = result + x * i
    return result
```
...
# 原因: 用了不支持的 Python 特�?
# 修复: 重写�?TorchScript 兼容代码，或改用 torch.jit.trace
```
...
```
优化�?
  %weight = prim::GetAttr(%self, "weight")  # 运行时查找属�?
  %bias = prim::GetAttr(%self, "bias")
  %1 = aten::linear(%x, %weight, %bias)

优化�?(freeze):
  %weight = prim::Constant(value=...)  # 编译时内联权�?
...
|------|-------------|---------------|
| 控制�?| 需要手写符合子集的代码 | 自动处理（Dynamo 字节码追踪） |
| 性能 | 仅图执行，无 kernel 融合 | Inductor 自动融合 + Triton codegen |
| 易用�?| 需改代码、处理报�?| `model = torch.compile(model)` 一�?|
| 动�?shape | 需重新 trace | Guard 自动切分 |

**结论**：PyTorch 2.0+ �?TorchScript 进入维护模式，新项目�?`torch.compile`�?

### A3.2.2 TorchScript 的遗�?

TorchScript 虽然被取代，但留下了重要遗产�?

- **IR 设计**：TorchScript IR（`torch::jit::Graph`）影响了 FX Graph �?Dynamo 的设�?
- **C++ 序列�?*：`.pt` 格式�?C++ 加载能力仍被使用
- **类型系统**：TorchScript 的类型推导思想延续�?`torch.jit.annotate`
- **Mobile 部署**：TorchScript Mobile 至今仍是 PyTorch 移动端方�?
...
ONNX（Open Neural Network Exchange）是一�?*开放的模型表示格式**。不同框架（PyTorch、TensorFlow、MXNet）都能导�?ONNX，不同推理引擎（ONNX Runtime、TensorRT、OpenVINO）都能加�?ONNX�?
...
ONNX 模型是一�?*有向无环图（DAG�?*，用 protobuf 序列化：
...
torch.onnx.export(
    ...
    example_input,
    "model.onnx",
    opset_version=17,        # ONNX 算子集版�?
    input_names=["input"],
    output_names=["output"],
    dynamic_axes={           # 支持动�?batch
        "input": {0: "batch"},
        "output": {0: "batch"}
    }
)
...
```
1. �?example_input 跑前�?�?记录算子调用序列
2. �?PyTorch 算子翻译�?ONNX 算子:
...
# 1. 创建 "tracer" 模式，拦截所�?aten:: 算子调用
# 2. 执行 model(example_input) �?记录算子序列
# 3. 对每个算子，�?symbolic_opset{N}.py 找到对应�?ONNX 符号函数
# 4. 符号函数�?aten 算子翻译�?ONNX NodeProto
# 5. 把所�?NodeProto 组装�?GraphProto �?ModelProto
# 6. protobuf 序列�?�?model.onnx
...
# �?�?aten::add(alpha) 翻译�?ONNX Mul + Add
```
...
| `torch.einsum` | 无直接对�?| 需拆分�?MatMul + Transpose |
| `torch.flip` | `Slice` | �?Slice 实现 |
| `torch.where` | `Where` | opset 9+ |
| `torch.gather` | `Gather` | opset 1+ |

**问题**：不是所�?PyTorch 算子都有 ONNX 对应。遇到不支持的算子，需要：
1. 注册自定�?ONNX 算子（`torch.onnx.register_custom_op`�?
2. 或用 `torch.jit.script` 替代 trace（保留控制流，但 ONNX 对控制流支持有限�?

### A3.3.6 自定义算子导�?

```python
# 自定义算�? my_hardswish(x) = x * relu6(x+3) / 6
...
torch.onnx.export(model, x, "model.onnx",
    dynamic_axes={"input": {0: "batch"}}  # �?0 维动�?
)
# 导出的模型支持任�?batch size
```

**动态轴的限�?*�?
- 只能指定**整维**为动态（不能"维度 1 �?3 �?4"�?
- 动态维度影响推理引擎优化（固定 shape 更好优化�?
- 某些算子（如 reshape 到固�?shape）与动态轴冲突

```python
# 常见动态轴设置
dynamic_axes = {
    "input":  {0: "batch", 2: "height", 3: "width"},  # 动�?batch + 分辨�?
    "output": {0: "batch"},
}

# 生产建议: 如果 batch 固定，不要设动�?�?推理引擎优化更好
```
...
**选择建议**：用推理引擎支持的最高版本（�?opset 算子更丰富，导出成功率高）�?

### A3.3.9 ONNX 控制�?

ONNX �?opset 11 起支持控制流算子�?
...
**限制**�?
- 控制流用子图表示，比 TorchScript �?`prim::If` 更重
...
ONNX Runtime 的图优化（融合、常量折叠、内存规划）�?`torch.compile` �?Inductor 类似，但作用�?ONNX IR 上�?
...
sess_options.optimized_model_file = "model_optimized.onnx"  # 保存优化后的�?
...
**优化级别**�?
- `ORT_DISABLE_ALL`：无优化
- `ORT_ENABLE_BASIC`：常量折叠、冗余算子消�?
- `ORT_ENABLE_EXTENDED`：算子融合（Conv+BN+ReLU �?ConvFusion�?
- `ORT_ENABLE_ALL`�? 布局转换、内存规�?
...
|------|-------------|------|---------------|
| 目标 | 脱离 Python 部署 | 跨框架交�?| 训练 + 推理加�?|
| IR | TorchScript IR | ONNX IR | FX Graph / Dynamo IR |
| 控制�?| script 支持 | 有限支持（If/Loop 算子�?| 自动（Dynamo�?|
| 序列�?| `.pt` (pickle) | `.onnx` (protobuf) | 不序列化（JIT 编译�?|
...
| 生产推理服务 | ONNX Runtime |
| 移动�?| TorchScript Mobile / torch.export |

---

## A3.6 torch.export：新一代导�?

PyTorch 2.1+ 引入 `torch.export`，替�?TorchScript 的导出功能：
...
### A3.6.2 torch.export �?IR

`torch.export` �?**ExportedProgram**，基�?FX Graph + ATen 算子�?
...
# 特点:
# - �?ATen 算子（无 prim::If，控制流已展开或用 higher-order op�?
# - 显式 shape 符号（batch 是符号，不是固定值）
# - 权重作为 graph 输入（不�?attribute），方便共享
```
...
# 原因: 用了 ONNX 不支持的算子
# 诊断: torch.onnx.export(..., verbose=True) 看哪个算子失�?
# 修复: 用等价的支持算子替换，或注册自定义算�?

# 错误 2: "Tracer encountered unsupported operation"
# 原因: 控制流或动态操�?
# 修复: �?torch.jit.script 先转 ScriptModule，再导出

# 错误 3: "dynamic_axes specified but dimension is fixed"
# 原因: 某操作固化了维度（如 reshape 到固�?shape�?
# 修复: 检�?reshape/view 操作，确保与动态轴兼容
...
onnx.checker.check_model(onnx_model)  # 不报�?= 合法

# 2. 对比 PyTorch �?ONNX Runtime 输出
model.eval()
with torch.no_grad():
...
np.testing.assert_allclose(
    ...
    rtol=1e-3, atol=1e-5  # ONNX �?fp32，误差应很小
)
print("导出验证通过�?)
```
...
minitorch �?Ch13 实现�?FX 风格的图捕获（`fx` 模块），可对照理解导出原理：
...
# �?ONNX export 的关�?
# - minitorch fx: 捕获图用于优�?编译（类�?torch.compile�?
# - ONNX export: 捕获�?+ 翻译�?ONNX 算子
# - 共同�? 都是 "trace 前向 �?记录算子 �?构建�?
```

**minitorch 未实�?ONNX 导出**（教学范围外），�?fx 模块展示�?图捕�?的核心思想，这是所有导出方案的基础�?
...
TensorRT �?NVIDIA 的高性能推理引擎，从 ONNX 构建�?

```
ONNX 模型 �?TensorRT Builder �?优化引擎 �?序列�?.engine �?部署
```
...
| 优化 | 效果 |
|------|------|
| 算子融合 | Conv+BN+ReLU �?1 �?kernel |
| 精度校准 | fp32 �?int8（用 ONNX 范围校准�?|
| Kernel auto-tuning | 选最�?kernel（针对当�?GPU�?|
| 内存规划 | 复用 buffer，减少分�?|
| Layer fusion | 横向融合（同层独立算子并行） |

**性能**：TensorRT 通常�?ONNX Runtime（CUDA）快 2-5×，因为做�?GPU 特定优化�?

### A3.10.3 其他推理引擎

| 引擎 | 平台 | 特点 |
|------|------|------|
| ONNX Runtime | CPU/GPU | 通用，易�?|
...
# 导出移动端模�?
scripted = torch.jit.script(model)
scripted = torch.jit.freeze(scripted)  # 消除可变属�?
scripted._save_for_lite_interpreter("model_lite.pt")

# 移动端加�?(C++)
# auto module = torch::jit::load("model_lite.pt");
// Lite Interpreter �?full TorchScript 轻量 10×
```

**移动端优�?*�?
- `torch.jit.freeze`：消除运行时属性查�?
- Lite Interpreter：精简的字节码解释�?
- 量化：int8 进一步减小模型体�?
...
**TorchScript** �?PyTorch 第一�?脱离 Python"方案，已�?`torch.compile` / `torch.export` 取代，但它的 IR 设计影响了后续方案�?

**ONNX** 是跨框架模型交换标准，至今活跃。导出流程是 trace + 算子翻译，推理用 ONNX Runtime / TensorRT 等专用引擎�?

**torch.export** 是新一代导出，支持动�?shape，与 torch.compile 共用 IR，是未来方向�?

三者的共同思想�?*把动�?Python 模型变成静态图**——这正是 Ch13 讲的"图与编译"的核心动机�?

**关键工程实践**�?
- 导出前先 `model.eval()`，固�?BN/Dropout 行为
- �?`onnx.checker.check_model` 验证导出合法�?
- �?`np.testing.assert_allclose` 对比 PyTorch �?ONNX 输出
- �?`onnxsim` 简化模型，减少冗余算子
- 选推理引擎支持的最�?opset 版本
