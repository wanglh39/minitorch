...
> 本附录对应主�?Ch3/Ch7/Ch9。Ch9 已实�?C++ Autograd Profiler，本附录深入讲解性能分析的方法论，并对照 PyTorch �?`torch.profiler`�?

---

## A4.1 为什么需�?profiler

训练慢的原因可能有很多，**不测量就不知道瓶颈在�?*�?

| 瓶颈类型 | 表现 | profiler 能发�?|
|---------|------|----------------|
| 算子�?| 某个算子耗时占比异常 | Node 执行时间排序 |
| 内存瓶颈 | 频繁 malloc/free �?OOM | 内存分配统计 |
| 通信瓶颈 | DDP 通信等待 | NCCL event 追踪 |
| 数据加载�?| GPU 空闲等数�?| DataLoader 时间占比 |
| Python 开销 | 算子调度�?| Python �?profiler |
...
## A4.2 minitorch �?C++ Profiler（Ch9 回顾�?
...
### A4.2.2 集成�?run_backward

�?`autograd/engine.cpp` 的每�?Node 执行前后记录�?
...
### A4.2.5 minitorch profiler 的实现细�?
...
- 记录内存差�?`mem_after - mem_before` 可发现分配热�?
...
记录每个 Node/算子的执行时间。我�?Ch9 实现的就是这一层�?
...
```
训练�?�?系统�?profiler: GPU 利用�?40%（GPU 空闲�?
       �?算子�?profiler: DataLoader �?50% 时间
       �?结论: 数据加载是瓶颈，不是计算
...
# 导出 Chrome Trace JSON（可�?chrome://tracing 可视化）
prof.export_chrome_trace("trace.json")
```

### A4.4.3 torch.profiler �?schedule
...
with torch.profiler.profile(
    ...
    schedule=torch.profiler.schedule(
        wait=2,     # �?2 步不记录（warmup�?
        warmup=2,   # 接下�?2 �?warmup（记录但不统计）
        active=6,   # 接下�?6 步正式记�?
        repeat=1,   # 重复 1 �?
    ),
    ...
    for step, (x, y) in enumerate(dataloader):
        ...
        loss.backward()
        optimizer.step()
        prof.step()  # �?必须调用，告�?profiler 进入下一�?
```

**schedule 参数**�?
- `wait`：跳过的步数（让缓存热起来）
- `warmup`：记录但不统计的步数（预�?profiler�?
- `active`：正式记录的步数
- `repeat`：重复整�?wait/warmup/active 循环

### A4.4.4 Chrome Trace 可视�?

`trace.json` 可在 `chrome://tracing` 打开，显�?*时间线视�?*�?
...
**关键洞察**：CPU �?CUDA 的时间线可以对比，发现：
- CPU 算子 dispatch 是否成为瓶颈（CPU 忙但 GPU 空闲�?
- kernel launch 延迟（CPU 发出�?GPU 还在等）
- CUDA stream 是否充分并行

### A4.4.5 record_shapes �?stack trace

```python
with torch.profiler.profile(
    record_shapes=True,       # 记录每个算子的输�?shape
    with_stack=True,          # 记录 Python 调用�?
...
| 发现 | 优化方法 |
|------|---------|
| matmul �?60% | 正常，无法优化（换更大矩�?batch�?|
| elementwise op �?20% | 算子融合（torch.compile�?|
| copy/cast �?10% | 减少 .cuda()/.cpu() 调用 |
| softmax �?5% | �?fused softmax（flash attention�?|
| dropout �?5% | �?fused dropout |
...
|------|------|------|
| 激活太�?| OOM | 梯度检查点（Ch9�?|
| 频繁分配 | malloc 占比�?| 预分�?buffer |
| 内存碎片 | 峰值高但平均低 | `torch.cuda.empty_cache()` |
| 梯度累积 | 梯度未清�?| `optimizer.zero_grad()` |
...
| 原因 | 表现 | 解决 |
|------|------|------|
| DataLoader �?| GPU 空闲等数�?| 增加 num_workers |
| CPU dispatch �?| CPU �?GPU �?| torch.compile |
| 频繁同步 | .item()/.cpu() 阻塞 | 减少同步 |
| batch 太小 | GPU 未打�?| 增大 batch |
| 算子太小 | kernel launch 开销�?| 算子融合 |
...
| 原因 | 含义 | 优化 |
|------|------|------|
| Memory Throttle | 等内�?| 改善访存模式（coalesced access�?|
| Compute Throttle | 计算单元�?| 无法优化（已打满�?|
| Not Selected | warp 调度未选中 | 增加 warp �?|
| Wait | 等依�?| 减少数据依赖 |

### A4.6.3 NVTX 标注

�?NVTX 在代码中标记区间，profiler 中可视化�?
...
**注意**：CUDA event 是异步的——`record()` 立即返回，`elapsed_time` 需�?`synchronize()` 后才准确�?
...
with torch.cuda.stream(s1):
    a = heavy_compute_1(x)  # �?stream 1
with torch.cuda.stream(s2):
    b = heavy_compute_2(y)  # �?stream 2（并行）
...
| `print(tensor)` | �?| 需要值才能打�?|
| `tensor < 0` (GPU) | �?| 仍在 GPU |
| `tensor.numpy()` | �?| 需 CPU 数据 |
| `if tensor > 0:` | �?| Python 需要标�?|
...
# 优化：torch.compile 自动融合�?1 �?kernel
d = torch.compile(lambda x, y: ((x * 2.0 + 1.0).relu() * y))(x, y)
```

**为什么融合快**�?
- 减少 kernel launch 开销�? �?�?1 次）
...
| 参数 | 作用 | 推荐�?|
|------|------|--------|
| `num_workers` | 数据加载进程�?| 4-8（CPU 核数的一半） |
| `pin_memory` | 锁页内存（加�?H2D 拷贝�?| True（GPU 训练�?|
| `persistent_workers` | 不每 epoch 重建进程 | True |
| `prefetch_factor` | 每个 worker 预加�?batch �?| 2（默认） |

### A4.7.4 不必要的梯度计算

```python
# 反模式：推理时未�?torch.no_grad()
model.eval()
for x in test_data:
    output = model(x)  # �?仍然建图！浪费内存和时间

# 优化
with torch.no_grad():  # �?不建图，省内�?加�?
    for x in test_data:
        output = model(x)
```

### A4.7.5 未使�?mixed precision
...
```python
# 当前显存使用
print(f"已分�? {torch.cuda.memory_allocated() / 1e9:.2f} GB")
print(f"已保�? {torch.cuda.memory_reserved() / 1e9:.2f} GB")
print(f"峰�?   {torch.cuda.max_memory_allocated() / 1e9:.2f} GB")
...
model.layer1 = checkpoint(model.layer1, x)  # 不存中间激�?

# 2. 清空缓存（解决碎片化�?
torch.cuda.empty_cache()  # 释放未使用的显存给系�?
...
with torch.profiler.profile(
    ...
    for x, y in dataloader:
        loss = model(x, y)     # DDP �?backward �?AllReduce
        loss.backward()
        optimizer.step()
        prof.step()
```
...
```
反向传播�?
  每个参数梯度算完�?�?AllReduce (NCCL)
  �?通信时间可能�?profiler 中显示为 "nccl:all_reduce"

常见问题:
  通信占比�?�?梯度分桶太小（增�?bucket_cap_mb�?
  通信和计算未重叠 �?开�?DDP find_unused_parameters=False
  某些 rank �?�?检查数据均匀�?
```
...
### A4.10.1 �?minitorch �?hook �?profiler

```python
# 基于 minitorch �?autograd hook 实现自定�?profiler
...
### A4.10.2 �?Python context manager
...
|------|-----------|---------|------|
| Profiler | `Profiler` �?| `torch.profiler.profile` | `torch/profiler/` |
| ProfileEvent | `ProfileEvent` | `Event` | `torch/profiler/event.py` |
| 算子�?| Ch9 实现 | `torch.autograd.profiler` | `torch/autograd/profiler.py` |
| Chrome Trace | 未实�?| `export_chrome_trace` | `torch/profiler/` |
| 内存统计 | `Allocator` 统计 | `profile_memory=True` | `c10::Allocator` |
| CUDA event | 未实�?| `ProfilerActivity.CUDA` | `torch/csrc/profiler/` |
| schedule | 未实�?| `torch.profiler.schedule` | `torch/profiler/profiler.py` |
| NVTX | 未实�?| `torch.cuda.nvtx` | `torch/csrc/cuda/Module.cpp` |
| TensorBoard | 未实�?| `tensorboard_trace_handler` | `torch/profiler/` |
...
|------------------------|------------------------|
| 轻量，直接集成在 Engine �?| 仅算子级，无 kernel �?|
| 同时记录内存分配 | 不支�?CUDA event |
...
### A4.13.1 案例 1: 训练速度慢，GPU 利用�?40%

```
症状: 训练 100 epoch 需�?10 小时，GPU 利用�?40%
诊断:
  1. nvidia-smi �?GPU 40%，CPU 90%
  2. profiler �?DataLoader �?50% CPU 时间
  3. 原因: num_workers=0，数据加载在主进�?
...
### A4.13.3 案例 3: DDP 通信�?30%

```
症状: 4 �?DDP，通信�?30% 时间
诊断:
  1. profiler �?nccl:all_reduce �?30%
  2. 大量小梯�?AllReduce（每个参数一次）

优化:
  1. 增大 bucket: DDP(model, bucket_cap_mb=25)
     �?多个小梯度合并成一次大 AllReduce
  2. 开�?overlap: DDP(model, gradient_as_bucket_view=True)
     �?通信和计算重�?
  �?通信降到 15%
```
...
诊断:
  ...
  2. 原因: backward 用了 Python 循环，未向量�?
...
profiler 的核心价值：**用数据驱动优化决�?*�?

三个层次�?
1. **算子�?*（我们实现了）：哪个 Node 慢？
2. **内核�?*（PyTorch torch.profiler）：算子内部时间花在哪？
3. **系统�?*（Nsight Systems）：硬件资源是否打满�?

优化流程：profiler 定位瓶颈 �?针对性优�?�?�?profiler 验证�?*不要盲目优化**�?

**关键工程实践**�?
- 先测量再优化，先测量再验证（Amdahl 定律�?
- GPU 利用率低 �?�?DataLoader / 同步 / batch size
- 显存 OOM �?梯度检查点 / 混合精度 / 清空缓存
- 通信�?�?增大 bucket / 优化拓扑 / overlap
- 算子�?�?torch.compile 融合 / �?fused kernel
- �?NVTX 标记代码区间，profiler 中定�?
